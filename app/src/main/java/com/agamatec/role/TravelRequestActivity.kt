        ...
        try {
            ...
            Log.d("TravelRequestActivity", "submitForm: Requisição construída, URL: $url")
                ...
                try {
                    ...
                    Log.d("TravelRequestActivity", "submitForm: Requisição enviada, aguardando resposta")

                    if (response.isSuccessful) {
                        val responseBody = response.body?.string()
                        Log.d("TravelRequestActivity", "submitForm: Requisição bem-sucedida: ${response.code}, Resposta: $responseBody")
                        ...
                        runOnUiThread {
                            Toast.makeText(this, "Erro ao enviar a requisição: ${response.message}", Toast.LENGTH_SHORT).show()
                        }
                    }
                } catch (e: IOException) {
                    Log.e("API_ERROR", "Exceção na requisição", e)
                    runOnUiThread {
                        Toast.makeText(this, "Erro ao conectar à API: ${e.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            }.start()
            ...
            Toast.makeText(this, "Erro ao processar o formulário: ${e.message}", Toast.LENGTH_SHORT).show()
        }
    }

    private fun navigateToNextSteps() {
        Log.d("TravelRequestActivity", "navigateToNextSteps: Navegando para a tela de próximos passos")
